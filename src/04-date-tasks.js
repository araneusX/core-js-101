...
function timeSpanToString(startDate, endDate) {
  ...
  return `${
    (`0${Math.floor(Math.floor(Math.floor(span / 1000) / 60) / 60) % 60}`).slice(-2)
  }:${
    (`0${Math.floor(Math.floor(span / 1000) / 60) % 60}`).slice(-2)
  }:${
    (`0${Math.floor(span / 1000) % 60}`).slice(-2)
  }.${
    (`00${span % 1000}`).slice(-3)
  }`;
}
...
function angleBetweenClockHands(date) {
  ...
  const result = (Math.min(360 - angle, angle) * Math.PI) / 180;
  return result;
}
